...
from rllearn.gail.run_mujoco import runner
from rllearn.gail.dataset.mujocodset import MujocoDset
...
def learn(env, policy_func, dataset, optim_batch_size=128, max_iters=1e4, adam_epsilon=1e-5, optim_stepsize=3e-4,
    ...
    loss = tf.reduce_mean(input_tensor=tf.square(action_ph - policy.ac))
