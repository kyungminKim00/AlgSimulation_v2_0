...
class DDPG(object):
    def __init__(self, input_dims, buffer_size, hidden, layers, network_class, polyak, batch_size,
        ...
        :param network_class: (str) the network class that should be used (e.g. 'rllearn.her.ActorCritic')
    ...
    def _create_network(self, reuse=False):
        ...
        # running averages
        with tf.compat.v1.variable_scope('o_stats') as scope:
            if reuse:
                scope.reuse_variables()
            self.o_stats = Normalizer(self.dim_obs, self.norm_eps, self.norm_clip, sess=self.sess)
        with tf.compat.v1.variable_scope('g_stats') as scope:
            if reuse:
                scope.reuse_variables()
        ...
        # networks
        with tf.compat.v1.variable_scope('main') as scope:
            if reuse:
                scope.reuse_variables()
            self.main = self.create_actor_critic(batch_tf, net_type='main', **self.__dict__)
            scope.reuse_variables()
        with tf.compat.v1.variable_scope('target') as scope:
            if reuse:
                scope.reuse_variables()
        ...
        self.q_loss_tf = tf.reduce_mean(input_tensor=tf.square(tf.stop_gradient(target_tf) - self.main.q_tf))
    ...
    def __setstate__(self, state):
        ...
        node = [tf.compat.v1.assign(var, val) for var, val in zip(_vars, state["tf"])]
        self.sess.run(node)
