...
def conv(input_tensor, scope, *, n_filters, filter_size, stride,
         pad='VALID', init_scale=1.0, data_format='NHWC', one_dim_bias=False, stride_method=None, use_BN=False):
    """
    ...
    else:  # use batch normalization - batch normalization take the charge of bias
        with tf.compat.v1.variable_scope(scope):
            weight = tf.compat.v1.get_variable("w", wshape, initializer=ortho_init(init_scale))

            return tf.nn.conv2d(input=input_tensor, filters=weight, strides=strides, padding=pad, data_format=data_format)
...
def find_shared_variables(key, t_key):
    with tf.compat.v1.variable_scope(key):
        _shared_variables = tf.compat.v1.trainable_variables()

    last_idx = 0
    for idx in range(len(_shared_variables)):
        if t_key in _shared_variables[idx].name:
            last_idx = idx

    return _shared_variables[last_idx], None


def find_moving_mean_varience(key):
    """
    Returns the batch normal variables within a given scope(except trainable variables)

    :param key: (str) The variable scope
    :return: ([TensorFlow Tensor]) the non-trainable statistics variables
    """
    with tf.compat.v1.variable_scope(key):
        return tf.compat.v1.global_variables()
    # with tf.variable_scope(key):
    #     return [item for item in tf.global_variables()
    #             if 'moving_mean' in item.name or
    #             'moving_variance' in item.name]
        # return [item for item in tf.global_variables()
        #         if 'beta' in item.name or
        #         'gamma' in item.name or
        #         'moving_mean' in item.name or
        #         'moving_variance' in item.name]
