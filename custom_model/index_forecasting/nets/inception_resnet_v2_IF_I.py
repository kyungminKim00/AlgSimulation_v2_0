...
def block_stem(inputs, scope=None, reuse=None):
    ...
    with tf.compat.v1.variable_scope(scope, 'Stem', [inputs], reuse=reuse):
        # 5 X 20 X 313
        net = slim.conv2d(inputs, 1024, [1, 1], scope='Conv2d_0a_1x1')
        net = slim.conv2d(net, 512, [1, 1], scope='Conv2d_0b_1x1')
    ...
    with slim.arg_scope(inception_arg_scope(use_batch_norm=True)):
        with slim.arg_scope([slim.conv2d, slim.fully_connected],
            ...
            with slim.arg_scope([slim.batch_norm, slim.dropout], is_training=is_training):
                ...
                # Inception
                # 5 x 10 x 384
                layer_1_5 = block_inception(layer_1_5, 'Inception1')
                layer_2_5 = block_inception(layer_2_5, 'Inception2')
                layer_3_5 = block_inception(layer_3_5, 'Inception3')

                # concat 5 x 5 x 1088
                net = tf.concat([layer_1_5, layer_2_5, layer_3_5], axis=3)

                # 5 x 5 x 3264
                net = slim.conv2d(net, 3264, 1, scope='Conv2d_5b_1x1')
