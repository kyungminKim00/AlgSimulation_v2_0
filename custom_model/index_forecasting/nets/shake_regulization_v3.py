...
def attn_layer(net, filters, kernel_size=1):
    x = slim.conv2d(net, filters, kernel_size)
    x = tf.nn.relu(x)

    score = slim.conv2d(x, 1, kernel_size, normalizer_fn=None, normalizer_params={})
    prob = tf.nn.softplus(score)

    # # l2 normalization before multiply - disable already done at stem layer
    # net = tf.nn.l2_normalize(net, axis=-1)
    # net = tf.reduce_mean(tf.multiply(net, prob), [1, 2], keepdims=False)
    return tf.multiply(net, prob)
