...
from custom_model.index_forecasting.common.utils import conv, linear, conv_to_fc, batch_to_seq, seq_to_batch, lstm
...
def block5(net, scale=1.0, activation_fn=tf.nn.relu, scope=None, reuse=None):
    ...
    with tf.compat.v1.variable_scope(scope, 'block5', [net], reuse=reuse):
        ...
        # Add batch Normalizer
        net = tf.compat.v1.layers.batch_normalization(
            inputs=net, momentum=0.997, epsilon=1e-5, fused=True, training=is_training)
