...
def block_stem(inputs, w_1, w_2, scope=None, reuse=None):
    ...
    with tf.compat.v1.variable_scope(scope, 'Stem', [inputs], reuse=reuse):
        ...
        net = slim.conv2d(net, w_1, [1, 1], activation_fn=tf.nn.relu, scope='Conv2d_0b_1x1')
...
def shakenet(scaled_images, is_training=False, **kwargs):
    ...
    with slim.arg_scope(shake_arg_scope(use_batch_norm=True)):
        with slim.arg_scope([slim.batch_norm, slim.dropout], is_training=is_training):
            ...
            # Stem
            # 5 X 20 X 150
            layer_1_5 = block_stem(scaled_images_1, 16, 24, 'Stem1')
            layer_2_5 = block_stem(scaled_images_2, 16, 24, 'Stem2')
            layer_3_5 = block_stem(scaled_images_3, 16, 16, 'Stem3')

            # concat 5 x 20 x 64
            net = tf.concat([layer_1_5, layer_2_5, layer_3_5], axis=3)
            net = cbam_block(net, 'CBAM_c', ratio=8)
            net = slim.conv2d(net, 32, [3, 3], scope='Conv2d_0cc_3x3')

            # 5 X 20 X 32
            net = block_residual_layer(net, 32, 5, [1, 1], is_training=is_training)
            # 5 X 20 X 64
            net = block_residual_layer(net, 64, 5, [1, 2], is_training=is_training)
            # 5 X 10 X 128
