...
from custom_model.index_forecasting.policies.base_class_index_forecasting import ActorCriticRLModel, \
    SetVerbosity, TensorboardWriter
from custom_model.index_forecasting.policies.policies_index_forecasting import LstmPolicy, ActorCriticPolicy
...
class A2C(ActorCriticRLModel):
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.compat.v1.variable_scope("loss", reuse=False):
                    ...
                    self.rewards_ph_2 = tf.compat.v1.placeholder(tf.float32, [None], name="rewards_ph_2")
                    self.learning_rate_ph = tf.compat.v1.placeholder(tf.float32, [], name="learning_rate_ph")
                    self.loss_bias_ph = tf.compat.v1.placeholder(tf.float32, [None, 3], name="loss_bias_ph")
                    ...
                    self.vf_loss_2 = mse(tf.squeeze(train_model.value_fn_2), self.rewards_ph_2)

                    if self.dynamic_coe:
                        minimum = tf.cast(tf.minimum(tf.minimum(self.pg_loss, self.vf_loss), self.vf_loss_2), dtype=tf.float32)

                        cons = tf.constant(1, dtype=tf.float32)
                        self.pi_coef = tf.cond(pred=tf.equal(self.pg_loss, minimum),
                                               true_fn=lambda: cons, false_fn=lambda: tf.divide(minimum, self.pg_loss))
                        self.vf_coef = tf.cond(pred=tf.equal(self.vf_loss, minimum),
                                               true_fn=lambda: cons, false_fn=lambda: tf.divide(minimum, self.vf_loss))
                        self.vf_coef_2 = tf.cond(pred=tf.equal(self.vf_loss_2, minimum),
                                                 true_fn=lambda: cons, false_fn=lambda: tf.divide(minimum, self.vf_loss_2))
                    else:
                        self.pi_coef = tf.convert_to_tensor(value=RUNHEADER.m_pi_coef, dtype=tf.float32)
                        self.vf_coef = tf.convert_to_tensor(value=RUNHEADER.m_vf_coef, dtype=tf.float32)
                        self.vf_coef_2 = tf.convert_to_tensor(value=RUNHEADER.m_vf_coef_2, dtype=tf.float32)

                    with ops.control_dependencies([self.pi_coef, self.vf_coef, self.vf_coef_2]):
                        barrier_coef = control_flow_ops.no_op(name='update_barrier_coef')

                    # reg_loss_1 = tf.add_n(slim.losses.get_regularization_losses())
                    reg_loss_2 = tf.reduce_sum(input_tensor=tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.REGULARIZATION_LOSSES))
                ...
                if update_ops:
                    ...
                    new_loss = control_flow_ops.with_dependencies([barrier, barrier_coef], loss)
                    new_loss_1 = control_flow_ops.with_dependencies([barrier, barrier_coef], loss_1)
                    new_loss_2 = control_flow_ops.with_dependencies([barrier, barrier_coef], loss_2)
                    new_loss_2_2 = control_flow_ops.with_dependencies([barrier, barrier_coef], loss_2_2)

                else:
                    new_loss = control_flow_ops.with_dependencies([barrier_coef], loss)
                    new_loss_1 = control_flow_ops.with_dependencies([barrier_coef], loss_1)
                    new_loss_2 = control_flow_ops.with_dependencies([barrier_coef], loss_2)
                    new_loss_2_2 = control_flow_ops.with_dependencies([barrier_coef], loss_2_2)

                # calculate gradient
                self.params = find_trainable_variables("model")
                self.ema_params = find_moving_mean_varience('model')  # just global variables
                if self.max_grad_norm is not None:
                    grads, _ = tf.clip_by_global_norm(tf.gradients(ys=new_loss, xs=self.params), self.max_grad_norm)
                    grads_pi, _ = tf.clip_by_global_norm(tf.gradients(ys=new_loss_1, xs=self.params), self.max_grad_norm)
                    grads_vf, _ = tf.clip_by_global_norm(tf.gradients(ys=new_loss_2, xs=self.params), self.max_grad_norm)
                    grads_vf_2, _ = tf.clip_by_global_norm(tf.gradients(ys=new_loss_2_2, xs=self.params), self.max_grad_norm)
                else:
                    ...
                    if self.full_tensorboard_log:
                        for partial_deviation, param in merge_grads:
                            if partial_deviation is not None:
                                tf.compat.v1.summary.scalar('partial_dev_' + param.name, tf.reduce_mean(input_tensor=partial_deviation))
                ...
                with tf.compat.v1.variable_scope("step_info", reuse=False):
                    ...
                    tf.compat.v1.summary.scalar('10day_return', tf.reduce_mean(input_tensor=self.returns_info_ph))
                ...
                with tf.compat.v1.variable_scope("input_info", reuse=False):
                    ...
                    if self.full_tensorboard_log:
                        ...
                        if tf_util.is_image(self.observation_space):
                            ...
                            tf.compat.v1.summary.scalar('processed_observation_mean', tf.reduce_mean(input_tensor=train_model.processed_obs))
                            tf.compat.v1.summary.scalar('processed_observation_sum', tf.reduce_sum(input_tensor=train_model.processed_obs))

                with tf.compat.v1.variable_scope("batch_parameter", reuse=False):
                    if self.full_tensorboard_log:
                        _list = ops.get_default_graph().get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES,
                                                                       'model/Stem_1_1/batch_normalization')
                        for item in _list:
                            tf.compat.v1.summary.scalar(item.name, tf.reduce_mean(input_tensor=item.value()))
                        _list = ops.get_default_graph().get_collection(tf.compat.v1.GraphKeys.GLOBAL_VARIABLES,
                ...
                if update_ops:
                    ...
                    train_op = control_flow_ops.with_dependencies([grad_updates], new_loss)
                ...
                with tf.compat.v1.variable_scope("assign_params", reuse=tf.compat.v1.AUTO_REUSE):
                    for tensor in self.ema_params:
                        self.weights_load_placeholder.append(
                            tf.compat.v1.placeholder(dtype=tensor.dtype, shape=tensor.get_shape()))
                        self.weights_load_op.append(tensor.assign(self.weights_load_placeholder[-1]))
        ...
        with SetVerbosity(self.verbose):
            ...
            with self.graph.as_default():
                ...
                with tf.compat.v1.variable_scope("loss", reuse=False):
                    ...
                    assert self.dynamic_coe is False, 'dynamic_coe should be false for gradnorm'
                    self.pi_coef = tf.convert_to_tensor(value=RUNHEADER.m_pi_coef, dtype=tf.float32)
                    self.vf_coef = tf.convert_to_tensor(value=RUNHEADER.m_vf_coef, dtype=tf.float32)
                    self.vf_coef_2 = tf.convert_to_tensor(value=RUNHEADER.m_vf_coef_2, dtype=tf.float32)

                    self.weight_pi_coef = tf.compat.v1.get_variable("weight_pi_coef", shape=1, initializer=tf.compat.v1.ones_initializer())
                    self.weight_vf_coef = tf.compat.v1.get_variable("weight_vf_coef", shape=1, initializer=tf.compat.v1.ones_initializer())
                    self.weight_vf_coef_2 = tf.compat.v1.get_variable("weight_vf_coef_2", shape=1,
                                                            initializer=tf.compat.v1.ones_initializer())

                    with ops.control_dependencies([self.pi_coef, self.vf_coef, self.vf_coef_2]):
                        barrier_coef = control_flow_ops.no_op(name='update_barrier_coef')

                    coef = tf.divide(3, self.weight_pi_coef + self.weight_vf_coef + self.weight_vf_coef_2)
                    with ops.control_dependencies([coef]):
                        renormalizing = control_flow_ops.no_op(name='update_renormalizing_coef')

                    reg_loss_1 = tf.reduce_sum(input_tensor=tf.compat.v1.get_collection(tf.compat.v1.GraphKeys.REGULARIZATION_LOSSES))
                    loss_1 = self.pg_loss * self.pi_coef * (1 - tf.exp(-tf.reduce_mean(input_tensor=self.weight_pi_coef * coef))) \
                             - self.entropy * self.ent_coef
                    loss_2 = self.vf_loss * self.vf_coef * (1 - tf.exp(-tf.reduce_mean(input_tensor=self.weight_vf_coef * coef)))
                    loss_2_2 = self.vf_loss_2 * self.vf_coef_2 * (1 - tf.exp(-tf.reduce_mean(input_tensor=self.weight_vf_coef_2 * coef)))
                    # loss = tf.divide(tf.add(tf.add(loss_1, loss_2), loss_2_2), 3)
                    loss = loss_1 + loss_2 + loss_2_2 + reg_loss_1

                    self.loss_bias = tf.stack([[loss_1, loss_2, loss_2_2]])  # current batch loss
                ...
                if update_ops:
                    with ops.control_dependencies(update_ops):
                        barrier = control_flow_ops.no_op(name='update_barrier')
                    new_loss = control_flow_ops.with_dependencies([barrier, barrier_coef, renormalizing], loss)
                    new_loss_1 = control_flow_ops.with_dependencies([barrier, barrier_coef, renormalizing], loss_1)
                    new_loss_2 = control_flow_ops.with_dependencies([barrier, barrier_coef, renormalizing], loss_2)
                    new_loss_2_2 = control_flow_ops.with_dependencies([barrier, barrier_coef, renormalizing], loss_2_2)

                else:
                    new_loss = control_flow_ops.with_dependencies([barrier_coef, renormalizing], loss)
                    new_loss_1 = control_flow_ops.with_dependencies([barrier_coef, renormalizing], loss_1)
                    new_loss_2 = control_flow_ops.with_dependencies([barrier_coef, renormalizing], loss_2)
                    new_loss_2_2 = control_flow_ops.with_dependencies([barrier_coef, renormalizing], loss_2_2)

                # calculate gradient
                self.params = find_trainable_variables("model")
                shared_params, non_shared_params = find_shared_variables("model", "Feature_out/weights")
                self.ema_params = find_moving_mean_varience('model')  # just global variables
                if self.max_grad_norm is not None:
                    grads, _ = tf.clip_by_global_norm(tf.gradients(ys=new_loss, xs=self.params), self.max_grad_norm)
                    grads_pi, _ = tf.clip_by_global_norm(tf.gradients(ys=new_loss_1, xs=shared_params), self.max_grad_norm)
                    grads_vf, _ = tf.clip_by_global_norm(tf.gradients(ys=new_loss_2, xs=shared_params), self.max_grad_norm)
                    grads_vf_2, _ = tf.clip_by_global_norm(tf.gradients(ys=new_loss_2_2, xs=shared_params),
                                                           self.max_grad_norm)
                else:
                    grads = tf.gradients(ys=new_loss, xs=self.params)
                ...
                grads_avg = tf.divide(tf.add(tf.add(grads_pi_norm, grads_vf_norm), grads_vf_2_norm), 3)
                ...
                loss_avg_hat = tf.divide(tf.add(tf.add(loss_1_hat, loss_2_hat), loss_2_2_hat), 3)
                ...
                l_grad_pi_norm = tf.compat.v1.losses.absolute_difference(grads_pi_norm, c1, reduction=reduction_method)
                l_grad_vf_norm = tf.compat.v1.losses.absolute_difference(grads_vf_norm, c2, reduction=reduction_method)
                l_grad_vf_2_norm = tf.compat.v1.losses.absolute_difference(grads_vf_2_norm, c2_2, reduction=reduction_method)
                ...
                with tf.compat.v1.variable_scope("grad_norm", reuse=False):
                    ...
                    tf.compat.v1.summary.scalar('weight_pi_coef', tf.reduce_mean(input_tensor=self.weight_pi_coef))
                    tf.compat.v1.summary.scalar('weight_vf_coef', tf.reduce_mean(input_tensor=self.weight_vf_coef))
                    tf.compat.v1.summary.scalar('weight_vf_coef_2', tf.reduce_mean(input_tensor=self.weight_vf_coef_2))
                    # tf.summary.scalar('coef', coef)
                    tf.compat.v1.summary.scalar('atual_weight_pi_coef', (1 - tf.exp(-tf.reduce_mean(input_tensor=self.weight_pi_coef * coef))))
                    tf.compat.v1.summary.scalar('atual_weight_vf_coef', (1 - tf.exp(-tf.reduce_mean(input_tensor=self.weight_vf_coef * coef))))
                    tf.compat.v1.summary.scalar('atual_weight_vf_coef_2', (1 - tf.exp(-tf.reduce_mean(input_tensor=self.weight_vf_coef_2 * coef))))
                    tf.compat.v1.summary.scalar('previous_loss_bias_pi', self.loss_bias_ph[-1, 0])
                    tf.compat.v1.summary.scalar('previous_loss_bias_vf', self.loss_bias_ph[-1, 1])
                    tf.compat.v1.summary.scalar('previous_loss_bias_vf2', self.loss_bias_ph[-1, 2])

                grads_norm_params = [self.weight_pi_coef, self.weight_vf_coef, self.weight_vf_coef_2]
                ...
                trainer = tf.compat.v1.train.RMSPropOptimizer(learning_rate=self.learning_rate_ph, decay=self.alpha,
                                                    epsilon=self.epsilon)
                trainer_2 = tf.compat.v1.train.RMSPropOptimizer(learning_rate=0.001, decay=self.alpha, epsilon=self.epsilon)
                if update_ops:
                    grad_updates = trainer.apply_gradients(merge_grads)
                    # Ensure the train_tensor computes grad_updates.
                    train_op = control_flow_ops.with_dependencies([grad_updates], new_loss)
                    ...
                    # Ensure the train_tensor computes grad_updates.
                    train_op2 = control_flow_ops.with_dependencies([grad_updates2], total_l_grad_norm)
                    # Add the operation used for training to the 'train_op' collection
    ...
    def weight_load_ph(self, weights):
        for load_op, load_placeholder, weight in zip(self.weights_load_op, self.weights_load_placeholder, weights):
    ...
    def _train_step(self, obs, states, rewards, masks, actions, values, update, writer=None,
        ...
        if RUNHEADER.m_online_buffer:
            ...
            if RUNHEADER.dynamic_lr:
                ...
                if RUNHEADER.predefined_fixed_lr is None:
                    ...
                    elif update <= 1000:  # 5(141samples // 32batch + 1)  * 20buffer * 10epoch
                        self.cur_lr = init_lr  # warm-up
                    else:
                        if RUNHEADER.target_name not in ['US10YT', 'GB10YT', 'DE10YT', 'KR10YT', 'CN10YT', 'JP10YT',
                                                         'BR10YT']:
                            ...
                            # use regression up/down performance = 0.65)
                                    ...
                                    explained_var) <= 0.85:  # fix condition later on (use regression up/down performance)
                                self.cur_lr = RUNHEADER.predefined_fixed_lr[1]
                            else:  # find global optimal on the recent local optimal
                                self.cur_lr = RUNHEADER.predefined_fixed_lr[2]

        run_params = [self.summary, self.pg_loss, self.vf_loss, self.entropy,
                      self.apply_backprop, self.vf_loss_2,
                      self.pi_coef, self.vf_coef, self.vf_coef_2, self.apply_backprop_gradnorm, self.loss_bias]
        td_map = {self.train_model.obs_ph: obs, self.actions_ph: actions, self.advs_ph: advs,
                  ...
                  self.returns_info_ph: returns_info, self.hit_ph: hit, self.rewards_ph_2: returns_info,
                  self.loss_bias_ph: np.expand_dims(np.mean(self.stack_loss_bias, axis=0), axis=0)}
        ...
        if writer is not None:
            ...
            if self.full_tensorboard_log and (1 + update) % self.tensorboard_log_update == 0:
                ...
                summary, policy_loss, value_loss, policy_entropy, _, value_loss2, pi_coef, vf_coef, vf_coef_2, _, loss_bias = \
                    self.sess.run(run_params, td_map, options=run_options, run_metadata=run_metadata)
            ...
            policy_loss, value_loss, policy_entropy, _, value_loss2, pi_coef, vf_coef, vf_coef_2 = \
                self.sess.run([self.pg_loss, self.vf_loss, self.entropy, self.apply_backprop, self.vf_loss_2,
                               self.pi_coef, self.vf_coef, self.vf_coef_2, self.apply_backprop_gradnorm], td_map)
        # previous loss
    ...
    def offline_learning_with_buffer(self, runner, writer, model_location):
        ...
        if RUNHEADER.on_cloud:
            ...
            buffer_dict = \
                {buffer_names[idx]: loadFile(RUNHEADER.m_offline_buffer_file + '/' + buffer_names[idx][:-4])
                 for idx in range(len(buffer_names))}
            ...
            for epoch in range(RUNHEADER.m_offline_learning_epoch):
                ...
                for buffer_name in buffer_names:
                    buffer = loadFile(RUNHEADER.m_offline_buffer_file + '/' + buffer_name[:-4])
                    print_out_csv_tmp, print_out_csv_colname = self.offline_learn(writer, runner, buffer,
                                                                                  model_location, epoch)
                    if print_out_csv is None:
                        print_out_csv = np.array(print_out_csv_tmp)
                    else:
                        if len(print_out_csv_tmp) != 0:
                            ...
                            print('{} - this buffer have been skipped. need more samples for the training'.format(
                                buffer_name))
                epoch_print_out_csv.append(np.mean(np.array(print_out_csv), axis=0).tolist())
            pd.DataFrame(epoch_print_out_csv, columns=print_out_csv_colname). \
                to_csv('{}/record_tabular_epoch_summary.csv'.format(model_location))
    ...
    def learn(self, total_timesteps, callback=None, seed=None, log_interval=10, tb_log_name="A2C",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            if RUNHEADER.m_online_buffer:
                ...
                self.learning_rate_schedule = Scheduler(initial_value=self.learning_rate, n_values=learning_timestemp,
                                                        schedule=self.lr_schedule)
            else:
                learning_timestemp = self.n_envs * self.n_steps * self.total_example * RUNHEADER.m_offline_learning_epoch
                self.learning_rate_schedule = Scheduler(initial_value=RUNHEADER.m_offline_learning_rate,
            ...
            for update in range(1, total_timesteps // self.n_batch + 1):
                ...
                policy_loss, value_loss, policy_entropy, value_loss2, pi_coef, vf_coef, vf_coef_2 = \
                    self._train_step(obs, states, rewards, masks, actions, values,
                                     ...
                                     returns_info, hit)
                self.pg_loss_bias, self.vf_loss_bias, self.vf_loss_2_bias = policy_loss, value_loss, value_loss2
                ...
                if update % int(self.total_example // 200) == 0:
                    model_name = '{}/fs_{}_ev{:3.3}_pe{:3.3}_pl{:3.3}_vl{:3.3}.pkl'.format(model_location,
                                                                                           current_timesteps,
                                                                                           explained_var,
                                                                                           policy_entropy,
                                                                                           policy_loss,
                ...
                # buffer save
                if (buffer.num_in_buffer >= buffer.size) or (update == self.total_example - 1):
                    writeFile('{}/buffer_E{}_S{}_U{}'.format(RUNHEADER.m_offline_buffer_file, self.n_envs, self.n_steps,
                                                             update), buffer)
                    buffer = None
                ...
                explained_var = self.print_out_tabular(update=update, values=values, rewards=rewards,
                                                       ...
                                                       value_loss2=value_loss2, pi_coef=pi_coef,
                                                       vf_coef=vf_coef, vf_coef_2=vf_coef_2)
        ...
        if (self.on_validation is True) and (epoch >= RUNHEADER.m_validation_min_epoch):
            ...
            with util.restore_agents_status(self):
                ...
                while np.sum(np.array(self.env.get_attr('eof'))) == 0:
                    ...
                    # memory growing
                    action, values, states, neglogp, values2 = runner.step_validation(obs, initial_state,
                                                                                      state=p_states,
                    ...
                    tmp_info = plot_util.gather_validation_performence(
                        info, tmp_info, values, values2, softmax_actions, index_bound, index_bound_return,
                        b_info, b_info_return)
                # plot and save performance
                plot_util.plot_save_validation_performence(tmp_info, model_name[:-len(
                    model_name.split('/')[-1]) - 1] + '/validation',
                                                           model_name)
    ...
    def retrive_experiences(self, buffer, runner, wrs=None):
        ...
        if wrs is None:  # original version
            ...
            obs_buffer, action_buffer, rewards_buffer, _, _, dones_buffer, _, _, \
            _, return_info_buffer, _ = buffer.get_non_replacement(wrs)
        ...
        for print_epoch_result in range(RUNHEADER.m_sub_epoch):
            ...
            for update in range(samples_number + 1):  # learning for 1 epoch
                ...
                if r_idx[-1] >= buffer.num_in_buffer:
                    ...
                    r_idx[-dummy_idx:] = np.random.randint(0, buffer.num_in_buffer, dummy_idx)
                ...
                self.pg_loss_bias, self.vf_loss_bias, self.vf_loss_2_bias = policy_loss, value_loss, value_loss2
                explained_var = self.print_out_tabular(policy_entropy=policy_entropy,
                ...
                print_out_csv.append([epoch, update, policy_entropy, policy_loss,
                                      value_loss,
                                      float(np.mean(np.reshape(rewards, [buffer.n_env, self.n_steps]))),
                                      float(np.mean(np.reshape(values, [buffer.n_env, self.n_steps]))), explained_var])
                print_out_csv_colname = ['epoch', 'update', 'policy_entropy', 'policy_loss', 'value_loss',
                                         'rewards', 'values', 'explained_var']

                # sys.stdout.write('\r>> [{}] Offline Learning step: {}/{}'.format(epoch, update, samples_number))
                sys.stdout.write('\r>> [{}] Offline Learning step: {}/{}'.format(epoch, (update + 1) * buffer.n_env,
                                                                                 buffer.num_in_buffer))
                sys.stdout.flush()

            # otherwise too much model are dropped. configure it according to the experimental result
            if print_epoch_result == (RUNHEADER.m_sub_epoch - 1) and self.epoch_track <= epoch:
                self.epoch_track = self.epoch_track + 1
                # model drop
                # Drop the model
                d_time = str(datetime.datetime.now())[:-10].replace(':', '-').replace('-', '').replace(' ', '_')
                model_name = '{}/{}_sub_epo_{}_pe{:3.3}_pl{:3.3}_vl{:3.3}_ev{:3.3}.pkl'.format(model_location, d_time,
                                                                                               epoch,
                                                                                               np.mean(np.array(
                                                                                                   print_out_csv)[:,
                                                                                                       2]),
    ...
    def save(self, save_path):
        ...
        ema_params = self.sess.run(self.ema_params)  # save ema_params updated 2020-02-18
    ...
    def _short_term_simulation(self, rewards, masks, hit, actions):
        ...
        cond_idx = np.squeeze(
            np.argwhere(np.array([util.consecutive_true(item) for item in cond]) > (self.n_steps - 1)),
            axis=1).tolist()
    ...
    def print_out_tabular(self, update=None, values=None, rewards=None, policy_entropy=None,
                          ...
                          value_loss2=None, pi_coef=None, vf_coef=None, vf_coef_2=None):
        """Write printout and save csv
        """
        if opt == 0:  # default tabular
            ...
            if RUNHEADER._debug_on:
                tabular_column_name = ['n_updates', 'total_timesteps', 'learning_rate', 'policy_entropy',
                                       ...
                                       'Episode_Dones', 'Episode_Rewards', 'values',
                                       'current_buff_size', 'learned_replay_ep',
                                       'non_pass_episode', 'date', 'short_term_roll_out']
                self.record_tabular.append([str(update) + '/' + str(self.total_example), self.num_timesteps,
            ...
            if RUNHEADER._debug_on:
                tabular_column_name = ['learning_rate', 'policy_entropy',
                                       ...
                                       'advs', 'convergence', 'policy_loss_with_coef', 'value_loss_with_coef',
                                       'value_loss_with_coef_2', 'value_loss2']
                ...
                self.record_tabular.append([self.cur_lr, float(policy_entropy), float(policy_loss),
                                            ...
                                            (policy_loss * pi_coef - policy_entropy *
                                             RUNHEADER.m_ent_coef + value_loss * vf_coef + value_loss2 * vf_coef_2),
                                            float(values), float(rewards),
                                            float(rewards - values), np.abs(float(policy_loss - values)),
...
class A2CRunner(AbstractEnvRunner):
        ...
        while b_step:
            ...
            if step_memory.num_in_buffer == self.n_env:
                ...
                sample_th, terminate_cnt, search_cnt = \
                    self.get_sample_th(search_cnt, terminate_cnt, step_memory)
    ...
    def step_simulation_offline(self, n_steps_idx=None, obs_buffer=None, action_buffer=None,
        ...
        _, values, states, _, _ = self.model.step(self.obs, self.states, self.dones, sample_th=sample_th)
    ...
    def step_validation(self, observation, initial_state, state, mask, deterministic=True):
        if state is None:
            state = initial_state
        if mask is None:
