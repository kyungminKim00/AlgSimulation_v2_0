...
def debug_session(num_cpu=None, make_default=False, graph=None, port=None, mode=0):
    if num_cpu is None:
        num_cpu = int(os.getenv('RCALL_NUM_CPU', multiprocessing.cpu_count()))
    tf_config = tf.compat.v1.ConfigProto(
        allow_soft_placement=True,
    ...
    if make_default:
        ...
        if mode==0:
            sess = tf.compat.v1.Session(config=tf_config, graph=graph)
            return tf_debug.TensorBoardDebugWrapperSession(sess, port, send_traceback_and_source_code=False)
        else:
            sess = tf.compat.v1.Session(config=tf_config, graph=graph)
