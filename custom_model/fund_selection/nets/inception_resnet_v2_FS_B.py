...
def inception_resnet_v2(scaled_images, is_training = False, **kwargs):
    with slim.arg_scope([slim.conv2d], padding='SAME', weights_initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=1.0, mode="fan_avg", distribution="uniform")):
        # Stem
        net = conv(scaled_images, 'Stem_1', n_filters=32, filter_size=3, stride=2, init_scale=np.sqrt(2), pad='SAME',
                   stride_method='Custom', **kwargs)
        net = conv(net, 'Stem_2', n_filters=32, filter_size=3, stride=2, init_scale=np.sqrt(2), pad='SAME',
                   stride_method='Custom', **kwargs)
        net = slim.conv2d(net, 64, 3, scope='Stem_3')
        net = slim.conv2d(net, 192, 3, scope='Stem_4')
        ...
        with tf.compat.v1.variable_scope('Feature_out'):
            ...
            net = slim.dropout(net, 0.8, is_training=True,
                               scope='Dropout')
            net = slim.fully_connected(net, 512*RUNHEADER.m_num_features, activation_fn=None, scope='Feature_out')
