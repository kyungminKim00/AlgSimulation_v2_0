...
class Script:
    ...
    def run(self, mode, env_name=None, tensorboard_log=None,
        ...
        self._inference(models, env_name, n_cpu, mode, model_location, result, m_inference_buffer)

    def _inference(self, models, env_name, n_cpu, mode, model_location, result, m_inference_buffer):
        # import modules
        ...
        if (RUNHEADER.m_final_model is None) or (RUNHEADER.m_final_model is 'None'):
            ...
            [filenames.append(_model) for _model in models if RUNHEADER.m_final_model in _model]
        ...
        for _model in filenames:
            ...
            if not is_graph_def_loaded:  # reset graph every 10times, to avoid memory overflow - fixed??? check
                # self.base_model = A2C.inf_init(_model, policy_kwargs={'n_lstm': 256 * RUNHEADER.m_lstm_hidden,
                #                                                                 'is_training': False})
                self.base_model = A2C.inf_init(_model, env=env, policy_kwargs={'n_lstm': int(256 * RUNHEADER.m_lstm_hidden),
                                                                      'is_training': False})
            ...
            while np.sum(np.array(env.get_attr('eof'))) == 0:
                ...
                """
                action, states, values, neglogp, values2 = self.base_model.predict(obs, state=p_states,
                ...
                b_info_return = list()
                _info = None
                if (current_step >= m_inference_buffer) and RUNHEADER.m_bound_estimation:
                    for sub_current_step in range(m_inference_buffer):
                        if sub_current_step > 0:
                            states = None
                            _values = None
                            _action = np.ones((env.num_envs, env.action_space.shape[0]), dtype=np.ndarray)
                            for inner_step in range(sub_current_step - 1, -1, -1):
                                ...
                                _action, states, _values, _, _values2 = self.base_model.predict(obs, state=states,
                                                                                                mask=None,
                                                                                                deterministic=True)
                            _, _, _, _info = env.step(_action)
                            b_info.append(((_info[0]['today_index'] * _values[0]) / 100) + _info[0]['today_index'])
                            b_info_return.append(_values[0])
                    index_bound = [np.min(np.array(b_info)), np.max(np.array(b_info)),
                                   np.mean(np.array(b_info)), np.std(np.array(b_info)), np.median(np.array(b_info))]
                    index_bound_return = [np.min(np.array(b_info_return)), np.max(np.array(b_info_return)),
                                          np.mean(np.array(b_info_return)), np.std(np.array(b_info_return)),
            ...
            """
            plot_util.plot_save_validation_performence(tmp_info, result, _model, split_name=mode)
...
def get_model_from_meta_repo(target_name, forward):
    model_info = load('./save/model_repo_meta/{}_T{}.pkl'.format(target_name, forward), 'pickle')
...
def configure_header(args):
    if bool(args.operation_mode):
        json_location, f_test_model = \
            get_model_from_meta_repo(RUNHEADER.target_id2name(args.m_target_index), str(args.forward_ndx))
    ...
    # Todo: re-write HEADER if necessary
    if bool(args.operation_mode):
        RUNHEADER.__dict__['dataset_version'] = args.dataset_version
        RUNHEADER.__dict__['m_dataset_dir'] = './save/tf_record/index_forecasting/if_x0_20_y{}_{}'.format(args.forward_ndx, args.dataset_version)
...
def meta_info(_model_location, _dataset_dir):
    ...
    infer_set = meta_2['verbose']  # infer_set contains one of [train | validation | test]
...
def naive_filter(model_list):
    ...
    for model_name in model_list:
        ...
        try:
            if float(token[5][2:]) < c_pe and float(token[6][2:]) < c_pl and \
                    float(token[7][2:]) < c_vl and float(token[8][2:-4]) > c_ev and \
                    int(token[4]) >= c_epoch:
