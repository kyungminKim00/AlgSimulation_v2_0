...
if __name__ == '__main__':
    try:
        ...
        parser.add_argument('--on_cloud', type=int, required=True)  # for debug test, load chunks of samples or all samples
        ...
        # parser.add_argument('--process_id', type=int, default=40)
        # parser.add_argument('--on_cloud', type=int, default=0)  # for debug test, load chunks of samples or all samples
        # parser.add_argument('--dataset_version', type=str, default='v10')
        ...
        if bool(args.ref_pid):
            ...
            if args.ref_pid == args.process_id:
                ...
                shutil.copy2('./buffer_generate_model_p{}.txt'.format(args.ref_pid),
        ...
        pickable_header = index_forecasting_train.convert_pickable(RUNHEADER)  # win32 only support spawn method for multiprocess unlike linux
        ...
        target_list = [validation, validation + '/fig_index', validation + '/fig_bound', validation + '/fig_scatter',
                       validation + '/fig_index/index', validation + '/fig_index/return', validation + '/fig_index/analytics']
        ...
        if RUNHEADER.m_online_buffer:  # Generate Buffer
            if args.search_variables:  # random pick x variables
                ...
                shutil.copy2(RUNHEADER.m_dataset_dir + '/x_index.json', _model_location + '/selected_x_dict.json')
        else:  # Learning with Buffer
            copy_file = ['/selected_x_dict.json']
            [shutil.copy2('./save/model/rllearn/' + json_location + file_name, _model_location + file_name) for file_name in copy_file]
        ...
        _file_pattern = str(RUNHEADER.l_objective) + '_' + str(RUNHEADER.dataset_version) + '_cv%02d_%s.pkl'  # data set file name
        ...
        # dataset injection
        if RUNHEADER.m_on_validation is True:
            sc = index_forecasting_train.Script(so=manager.DataSet(
                ...
                so_validation=manager.DataSet_Validation(
                    dataset_dir=_dataset_dir, file_pattern=_file_pattern,
                    split_name='validation', cv_number=_cv_number)
            )
        else:
            sc = index_forecasting_train.Script(so=manager.DataSet(
                dataset_dir=_dataset_dir, file_pattern=_file_pattern,
